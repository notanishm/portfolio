...
import './InfiniteMenu3D.css';

// ReactBits Infinite Menu (3D) adapted for:
// - hash navigation (no router)
// - imperative focusing from the top PillNav
// - resume-only content (title/description optional)
...
void main() {
    ...
    worldPosition.xyz = radius * normalize(worldPosition.xyz) * uScaleFactor;
...
class InfiniteGridMenu {
  ...
  // Imperative focus used by the top PillNav.
  focusItem(itemIndex) {
    if (!this.control) return;
    if (!this.items.length) return;
    const target = ((itemIndex % this.items.length) + this.items.length) % this.items.length;

    let bestVertexIndex = null;
    let bestDot = -Infinity;

    for (let i = 0; i < this.instancePositions.length; i++) {
      if (i % this.items.length !== target) continue;
      ...
      const d = vec3.dot(worldPos, this.control.snapDirection);
      if (d > bestDot) {
        bestDot = d;
        bestVertexIndex = i;
      }
...
const InfiniteMenu3D = forwardRef(function InfiniteMenu3D(
  ...
  useImperativeHandle(ref, () => ({
    focusItem: (index) => {
      menuRef.current?.focusItem(index);
    },
  }));
  ...
  useEffect(() => {
    ...
    const handleActive = (idx) => {
      const item = resolvedItems[idx % resolvedItems.length];
      setActiveItem(item);
